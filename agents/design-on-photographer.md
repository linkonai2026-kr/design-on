...
## C-3-1. 상품 사진은 스톡으로 안 된다

**"이 가게 로고가 박힌 컵", "이 브랜드 패키지"는 Unsplash에 없다.** AI로 만들면 로고가 뭉개진다. 비슷한 남의 제품 사진을 억지로 넣지 마라. 쇼핑몰·공방·브랜드 페이지에서 실제로 자주 막히는 지점이다.

그 자리는 C-4(타이포 주도)로 처리하고, **산출물에 "상품 사진 필요"라고 적어라.** 부모 에이전트가 STEP 8-3에서 무료 목업 사이트를 안내하고, 사용자가 만들어 온 파일로 교체한다.
