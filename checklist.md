...
## 21. Vibrant Color 게시물 (추가 요청)

- [x] `DbbBFirDFGx` 슬라이드 7장 확인 — 팔레트 5세트, 각 4색 + HEX
- [x] **넣기 전 design-on 기준으로 실측** — 역할 배정 + 대비 계산
  - [x] 본문 대비 통과 4/5 (9.0 / 9.26 / 6.32 / 8.19), Crimson/Ocean Blue만 4.12로 미달
  - [x] muted 후보 없음 4/5, accent 대비 낮음 4/5
  - [x] 배경이 크림/베이지인 것 3/5 → `cream-palette` 위험
- [x] **기존 DB 기준선과 비교** — 68세트 중 17세트(25%)가 이미 본문 대비 미달, muted 없음 58%
- [x] 5세트 전부 추가(총 83세트). 새 것만 엄격히 걸면 기준이 일관되지 않는다
- [x] 각 세트 `note`에 실측 대비값과 주의사항 기록
- [x] **버그 발견·수정** — `pick.mjs`가 본문/배경 대비 미달을 경고하지 않았다
  - [x] `warn`에 추가. DB 17세트가 조용히 통과하고 있었다
  - [x] PLAYBOOK 1-3 경고 예시 갱신 + "첫 번째 경고가 제일 무겁다" 명시
  - [x] 회귀 테스트 신설 — 미달 팔레트는 경고, 통과 팔레트는 경고 없음
- [x] 문서 전반 숫자 갱신 (팔레트 78→83, 툴 883→894)
- [x] 테스트 9종 + 시각 14종 통과
