...
**배색 후보는 화면에 실제 색으로 보여드립니다.** HEX 코드만 나열하면 색을 상상할 수 없으니까요. 미리보기 화면이 브라우저에서 열리고, 거기서 A~F 후보의 색을 직접 보고 고르시면 됩니다.

```
어울리는 배색을 6개 골라 화면에 띄웠습니다. 색을 보고 골라주세요.

1. 배색을 골라주세요 (화면의 A~F 중에서)
   ...
   D. Champagne/Coffee — 샴페인 베이지 + 위스키 브라운
      따뜻하고 고급스럽습니다. 인테리어·브랜드에 잘 어울립니다.
   E. Alabaster Cream/Rosewood — 크림 + 빈티지 로즈우드
      부드럽고 정감 있습니다. 베이커리·공방에 어울립니다.
   F. Terracotta Clay/Olive — 테라코타 + 올리브
      자연스럽고 견고합니다. 공방·리빙 브랜드에 어울립니다.
...
이 저장소에 위 절차를 그대로 돌려 나온 결과물이 다섯 개 있습니다. 브라우저로 바로 열어볼 수 있습니다.

**다섯을 일부러 완전히 다르게 만들었습니다.** 색만 다른 게 아니라 **구조가 다릅니다.** 같은 도구로 만들어도 가게가 가진 자산에 따라 다른 얼굴이 나와야 하기 때문입니다.
...
| [`examples/salon/`](examples/salon/index.html) | 미용실 | **목차 주도. 첫 화면이 곧 시술표** | 지금 예약 전화 받는지 실시간 표시 |
...
라틴·숫자 서체는 [Fontshare](https://www.fontshare.com/)에서 가져옵니다. `1,280도`·`02-1234-5678` 같은 표기가 한글 서체의 약한 숫자로 나오지 않게 하려는 것입니다.

다만 **라틴 서체가 Fontshare 한 곳에 걸려 있는 것이 약점이었습니다.** 그쪽 CDN이 느리거나 막히면 숫자와 영문만 기본 서체로 떨어집니다. 한글은 Pretendard라 멀쩡해서 가격표와 전화번호만 무너지니 오히려 더 눈에 띕니다. 그래서 Google Fonts 다섯 개(Public Sans · Libre Franklin · Work Sans · Figtree · Mulish)를 폴백으로 넣어뒀습니다. 본문·UI 전용이고, 개성은 제목 서체에서 냅니다.
...
## 프리미엄 모드 — 고급스럽게, 인터랙티브하게

"고급스럽게", "인터랙티브하게", "모던하게", "애플처럼", "imweb처럼" 같은 말을 하면 프리미엄 모드가 켜집니다. "알아서 해줘"라고 해도 기본으로 켜집니다.

목표 품질은 [imweb 테마](https://imweb.me/theme/)와 [apple.com/kr](https://www.apple.com/kr/) 수준입니다. 대형 제품 히어로, "더 알아보기/방문하기" 2링크, 큼직한 여백, 스크롤에 반응하는 인터랙티브.

```bash
node scripts/pick.mjs premium --match "고급스럽게 세련되게"   # 레벨 판정 (off/basic/full)
node scripts/pick.mjs premium --id scroll-reveal              # 모티프 CSS/JS 조회
```

프리미엄은 장식이 아니라 행동입니다. 스크롤하면 내용이 살아나고, 호버하면 반응합니다. 단, 두 가지를 지킵니다.
...
| 데이터 | 수량 | 내용 |
|---|---|---|
| 디자인 툴·사이트 | **894개** | 이름, 무료·유료, 카테고리, 어울리는 업종, 공식 URL |
...
아래는 각 버전에서 무엇을 왜 바꿨는지 길게 적은 것입니다. 요약만 보시려면 [CHANGELOG.md](CHANGELOG.md)가 짧습니다.
