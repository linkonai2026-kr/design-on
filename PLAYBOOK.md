...
한 문장 요청에서 완성된 웹페이지까지 간다.

**질문은 딱 한 번, 묶어서 한다.** 팔레트 DB를 먼저 조회해 실제 후보를 손에 쥔 다음(STEP 2), 그 근거로 3개 이하의 질문을 한 번에 던진다. 답을 받으면 더 묻지 말고 끝까지 만든다. 빈손으로 "어떤 스타일 원하세요?"를 묻거나, 만드는 도중에 하나씩 되묻는 것은 금지다.
...
designon은 아래 스킬이 있으면 자동으로 위임한다. **없으면 그냥 건너뛴다. 설치를 요구하며 멈추지 마라.**

| 스킬 | 있으면 맡길 일 | 확인 방법 | 설치 |
|---|---|---|---|
| **ui-skills** | 컴포넌트 베이스라인, 모션 카테고리 라우팅 | `baseline-ui` 등 ui-skills 계열 존재 | 기본 자동 |
| **emil-design-eng** | 애니메이션 설계·검수 | `emil-design-eng`, `review-animations` 존재 | 기본 자동 |
| **lazyweb** | 업종별 실제 제품 화면 레퍼런스 수집 | `lazyweb-quick-search` 등 존재 | 선택 |
...
## STEP 2 — 팔레트 조회 후 한 번만 질문

**순서가 중요하다. 먼저 DB를 조회하고, 그 결과를 근거로 질문한다.** 빈손으로 "어떤 색을 원하세요?"라고 묻지 마라.

### 2-1. 먼저 조회한다

`{ROOT}/data/palettes.json`을 읽고 요청에 맞는 후보를 **3개 이내로** 추린다. 선택 기준은 2-3에 있다.

### 2-2. 한 번에 묶어서 질문한다

추린 후보를 실제 색상값과 함께 보여주며 **한 번의 질문 라운드로 끝낸다.** 답을 받은 뒤 추가 질문을 하지 마라.

질문은 **3개 이하**로 하고, 아래에서 고른다. 이미 요청에 답이 있는 항목은 빼라.

| 질문 | 선택지 예시 | 언제 묻나 |
|---|---|---|
| **배색** | 조회한 후보 2~3개를 팔레트 이름 + 대표 HEX와 함께 | 항상 (색상 지정이 있어도 계열 안에서 고르게) |
| **분위기** | 업종별로 대비되는 2~3개. 카페면 "차분하고 조용한 / 밝고 활기찬 / 빈티지 감성" | 요청에 분위기 언급이 없을 때 |
| **가장 중요한 목적** | "방문 유도 / 메뉴·가격 안내 / 브랜드 인상 / 예약·문의 접수" | 항상 |
| **실제 상호명** | 자유 입력 | 상호가 안 나왔을 때 |

선택지에는 **각각이 어떤 결과로 이어지는지** 한 줄 설명을 붙인다. 예: "차분하고 조용한 — 여백을 넓게 쓰고 사진은 적게, 명조체 제목".

Claude Code에서는 선택지를 제시하는 질문 도구를 쓴다. 그 외 환경에서는 번호를 매긴 목록으로 한 메시지에 묶어 보낸다. 어느 쪽이든 **사용자가 "1, 2, 상호는 ○○" 식으로 한 번에 답할 수 있어야 한다.**

### 2-3. 답을 받으면 즉시 끝까지 만든다

이후로는 묻지 않는다. 나머지 미정 항목은 전부 업종 통념으로 직접 정하고 STEP 3~6을 한 번에 실행한다.

**사용자가 "알아서 해줘"라고 답하거나 질문을 건너뛰면,** 후보 중 첫 번째 팔레트와 업종 표준값으로 바로 진행한다. 다시 묻지 마라.
